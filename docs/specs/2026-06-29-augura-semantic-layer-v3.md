...
The active semantic store is the Supabase `semantic` schema and the frontend bundle loaded at startup. Semantic release versioning, FK enforcement, and logical separation between taxonomy, causal ontology, and DQ constraints are maintained at the database level.
...
The current release is surfaced through a read-only admin workspace at the `/semantic` route. Its scope,
tab layout, and interaction model are specified separately in the Semantic Admin north-star
(`docs/specs/2026-06-26-semantic-admin-north-star.md`); this architecture document does not define the
tool's UI.
