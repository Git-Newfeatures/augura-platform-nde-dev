"""Power analytique (mode LIVE) — port fidèle de SimulationEngine.computeAnalytical.

Synchrone, sans dépendance scientifique (l'image API reste légère ; le bootstrap
numpy/scipy/statsmodels tourne en job Modal séparé). Calibré par outcome (fix T4)
via calibration.sigma_for.
"""
...
def normal_cdf(z: float) -> float:
    """Approximation Abramowitz-Stegun (identique au front pour des valeurs alignées)."""
...
def compute_power(
    *,
    ...
    """Power/biais/variance par estimateur. Lève si les groupes sont trop petits."""
    n_eff = n * (1 - dropout)
    n_treat = n_eff * calibration.TREAT_PROP
    n_ctrl = n_eff * (1 - calibration.TREAT_PROP)
    if n_treat < 5 or n_ctrl < 5:
        raise ValueError("groupes trop petits (n_treat ou n_ctrl < 5)")
