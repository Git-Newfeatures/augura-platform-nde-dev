...
def mint() -> str:
    now = int(time.time())
    return jwt.encode(
        {"sub": USER, "aud": "authenticated", "iat": now, "exp": now + 3600,
...
async def main() -> int:
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://t") as c:
        ok &= check("requête non authentifiée rejetée (401)",
    ...
    ok &= check("outcomes: hba1c_pct + regulatory_tags",
                "hba1c_pct" in by and bool(by["hba1c_pct"].get("regulatory_tags")))
    ok &= check("outcomes: pas de tag 'In dataset' seedé (dérivé de la cohorte)",
    ...
    ok &= check("estimators: lme eligible_study_types == [retro,prosp]",
                eby.get("lme", {}).get("eligible_study_types") == ["retro", "prosp"])
    ok &= check("estimators: ipw eligible_study_types == [retro]",
    ...
    ok &= check("cesl-sources: default_evidence_type exposé",
    ...
    ok &= check("dq-rules: pii_patterns + biomarker_ranges",
    ...
    ok &= check("variable-roles: groupes + alias (environment->engagement)",
                any(g.get("code") == "outcomes" for g in vr.get("groups", []))
                and (vr.get("group_aliases") or {}).get("environment") == "engagement")

    for p, n in [("/reference/estimands", 4), ("/reference/frameworks", 6),
                 ("/reference/evidence-types", 8), ("/reference/domains", 17),
                 ("/reference/jurisdictions", 7), ("/reference/literature-study-designs", 13)]:
        rows = data.get(p) or []
